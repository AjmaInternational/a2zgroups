...
const Home = () => {
  ...
  return (
    <div className="overflow-hidden">
      ...
      <section
        className="relative h-[60vh] md:h-[80vh] overflow-hidden group"
        onTouchStart={onTouchStart}
        onTouchMove={onTouchMove}
        onTouchEnd={onTouchEndPromo}
      >
        {promoBanners && promoBanners.length > 0 ? (
          promoBanners.map((banner, idx) => (
            <div
              ...
              className={`absolute inset-0 transition-all duration-1000 ease-in-out ${
                idx === currentPromoIndex ? 'opacity-100 scale-100' : 'opacity-0 scale-110 pointer-events-none'
              }`}
            >
              <img
                src={banner.image_url || "https://images.unsplash.com/photo-1490481651871-ab68de25d43d?auto=format&fit=crop&q=80&w=2000"}
                alt={banner.title}
                className="absolute inset-0 w-full h-full object-cover"
              />
              <div className="absolute inset-0 bg-primary-dark/50"></div>
              <div className="relative h-full flex flex-col items-center justify-center text-center px-6 z-10">
                <h4 className="text-primary font-bold tracking-[0.3em] mb-4 transform transition-all duration-700 delay-300 translate-y-8 opacity-0 group-hover:translate-y-0 group-hover:opacity-100" style={idx === currentPromoIndex ? {transform: 'translateY(0)', opacity: 1} : {}}>EXCLUSIVE OFFER</h4>
                <h2 className="text-5xl md:text-8xl text-white mb-8 max-w-4xl transform transition-all duration-1000 delay-500 translate-y-12 opacity-0 group-hover:translate-y-0 group-hover:opacity-100" style={idx === currentPromoIndex ? {transform: 'translateY(0)', opacity: 1} : {}}>{banner.title}</h2>
                <p className="text-white/80 text-xl md:text-2xl mb-12 max-w-2xl font-light transform transition-all duration-1000 delay-700 translate-y-16 opacity-0 group-hover:translate-y-0 group-hover:opacity-100" style={idx === currentPromoIndex ? {transform: 'translateY(0)', opacity: 1} : {}}>
                  {banner.subtitle}
                </p>
                <div className="transform transition-all duration-1000 delay-1000 translate-y-20 opacity-0 group-hover:translate-y-0 group-hover:opacity-100" style={idx === currentPromoIndex ? {transform: 'translateY(0)', opacity: 1} : {}}>
                  <Button to="/shop" variant="primary" size="lg" className="bg-white text-primary hover:bg-primary hover:text-white border-none rounded-full px-12 py-5 font-bold tracking-widest">
                    EXPLORE NOW
                  </Button>
                </div>
              </div>
          ...
          <div className="absolute inset-0 bg-primary-dark/40 flex flex-col items-center justify-center text-center px-6">
            <img
              src="https://images.unsplash.com/photo-1490481651871-ab68de25d43d?auto=format&fit=crop&q=80&w=2000"
              alt="Default Promo"
              className="absolute inset-0 w-full h-full object-cover -z-10"
            />
            <h2 className="text-5xl md:text-7xl text-white mb-8">SUMMER COLLECTION 2025</h2>
        ...
        {promoBanners && promoBanners.length > 1 && (
          <>
            <button
              onClick={() => setCurrentPromoIndex((prev) => (prev - 1 + promoBanners.length) % promoBanners.length)}
              className="absolute left-8 top-1/2 -translate-y-1/2 w-16 h-16 rounded-full border border-white/20 flex items-center justify-center text-white hover:bg-white hover:text-primary transition-all z-20 backdrop-blur-md opacity-0 group-hover:opacity-100"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
              </svg>
            ...
            <button
              onClick={() => setCurrentPromoIndex((prev) => (prev + 1) % promoBanners.length)}
              className="absolute right-8 top-1/2 -translate-y-1/2 w-16 h-16 rounded-full border border-white/20 flex items-center justify-center text-white hover:bg-white hover:text-primary transition-all z-20 backdrop-blur-md opacity-0 group-hover:opacity-100"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
              </svg>
              ...
              {promoBanners.map((_, idx) => (
                <button
                  ...
                  className={`h-1.5 transition-all duration-500 rounded-full ${
                    idx === currentPromoIndex ? 'w-16 bg-primary' : 'w-8 bg-white/30 hover:bg-white/50'
                  }`}
                />
              ))}
            </div>
